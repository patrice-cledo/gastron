    ...
    (set, get) => {
      ...
      return {
        ...
        addItems: (ingredients, recipeId, recipeTitle, servings = 4, sources) => {
          ...
          set((state) => {
            // Clone existing items to mutate
            const updatedItems = [...state.items];

            ingredients.forEach((ingredient, index) => {
              // Determine source for this ingredient
              const source: GrocerySource | undefined = sources
              ...
              // Try to find existing item with same name and unit
              const existingItemIndex = updatedItems.findIndex(item =>
                item.name.trim().toLowerCase() === normalizedName &&
                (item.unit === ingredient.unit || (!item.unit && !ingredient.unit))
              );
              ...
              if (existingItemIndex !== -1) {
                ...
                // Try to add amounts
                let newAmount = existingItem.amount;
                const currentVal = parseFloat(existingItem.amount);
                const addVal = parseFloat(ingredient.amount);

                if (!isNaN(currentVal) && !isNaN(addVal)) {
                  newAmount = String(currentVal + addVal);
                } else {
                  // Should we just append text? But the UI expects amounts. 
                  // Let's keep existing behavior if nan, or just use the new value? 
                  // Ideally we shouldn't get here with the new logic in IngredientSelectionScreen.
                }

                // Merge sources
                const currentSources = existingItem.sources || [];
                const newSources = source ? [...currentSources, source] : currentSources;

                updatedItems[existingItemIndex] = {
                  ...existingItem,
                  amount: newAmount,
                  sources: newSources,
                  // Uncheck if adding more? Usually yes.
                  checked: false,
                };
            ...
            console.log('🛒 Items merged/added. Total items:', updatedItems.length);
            ...
            set((state) => {
              ...
              return {
                recipes: [...state.recipes, {
                  id: recipeId,
                  title: recipeTitle, // Fixed title property name from 'name' to 'title' if needed, but keeping existing structure
                  image: undefined,
